...
describe("testing slit switch and slit size field", () => {
    test("testing slit switch field render with defaultValue and perfectly visible", () => {
        render(<App />);
        // change mode to make the slit switch visible
        const modeSelect = screen.getByTestId("mode-select-testid");
        fireEvent.mouseDown(modeSelect);
        const selectNode = modeSelect.childNodes[1];
        fireEvent.change(selectNode, { target: { value: "radiance_noslit" } });
